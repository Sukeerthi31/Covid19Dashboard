...
conn=sf.connect(
    ...
    schema=Variable.get("US_DAILY_CASES_SCHEMA")
    )
...
def carga_stage(conn, **kwargs):
    ...
    csv_file="/mnt/c/users/sm139/CSV/USA_Cases/*.csv"
...
def carga_tabla(conn, **kwargs):
    sql2 = "copy into {0} from @{1}/ FILE_FORMAT=(format_name=us_data) ON_ERROR = 'CONTINUE' ".format(Variable.get("US_DAILY_CASES_TABLE"),Variable.get("US_STAGE"))
    curs=conn.cursor()
    curs.execute(sql2)
