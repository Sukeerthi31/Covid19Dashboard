...
etl_dag = DAG( 'GLOBAL_VACCINATIONS_BY_AGE_CSV_UPLOAD',
...
def carga_stage(conn, **kwargs):
    ...
    csv_file='/mnt/c/csv/GlobalVaccination/vaccinations-by-age-group.csv'
...
def carga_table(conn, **kwargs):
    sql2 = "copy into {0} from @{1}/vaccinations-by-age-group.csv.gz FILE_FORMAT=(TYPE=csv field_delimIter=',' skip_header=1) ON_ERROR = 'CONTINUE' ".format(Variable.get("GLOBAL_VACCINATIONS_BY_AGE_TABLE"),Variable.get("GLOBAL_VSTAGE"))
    curs=conn.cursor()
    curs.execute(sql2)
