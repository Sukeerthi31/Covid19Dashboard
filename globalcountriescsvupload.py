...
def carga_stage(conn, **kwargs):
    ...
    csv_file='/mnt/c/csv/GlobalVaccination/countries.csv'
...
def carga_table(conn, **kwargs):
    sql2 = "copy into {0} from @{1}/countries.csv.gz FILE_FORMAT=(TYPE=csv field_delimIter=',' skip_header=1) ON_ERROR = 'CONTINUE' ".format(Variable.get("GLOBAL_COUNTRIES_TABLE"),Variable.get("GLOBAL_VSTAGE"))
    curs=conn.cursor()
    curs.execute(sql2)
